    ...
    <div className="min-h-screen w-full bg-[#0a0f1c] text-white">
      ...
      <header className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
        ...
        <nav className="hidden gap-6 text-sm md:flex">
          <a href="#missions" className="opacity-80 hover:opacity-100">Missions</a>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-12 pt-6">
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-3xl font-bold md:text-5xl">
        ...
        <p className="mt-4 max-w-xl text-white/80">
          Plateforme instantanée pour freelances: missions claires, paiement simple.
        </p>
        <div className="mt-6 flex items-center gap-2 rounded-2xl border border-white/10 bg-[#0e1526] px-4 py-3">
          <Search className="h-5 w-5 opacity-70" />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Rechercher une mission..." className="w-full bg-transparent text-sm outline-none placeholder:text-white/50" />
        </div>
      </section>
          ...
          {filtered.map((m) => (
            <motion.div key={m.id} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <article className="flex flex-col rounded-3xl border border-white/10 bg-white/5 p-4 backdrop-blur">
                ...
                <button onClick={() => setSelected(m)} className="mt-3 rounded-2xl bg-white px-4 py-2 text-sm font-semibold text-[#0a0f1c]">Voir / Postuler</button>
              </article>
            </motion.div>
          ))}
        </div>
      ...
      <AnimatePresence>
        {selected && (
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }} className="relative z-10 max-w-lg w-full rounded-3xl border border-white/10 bg-[#0e1526] p-6">
              <h3 className="text-xl font-semibold">{selected.title}</h3>
              <p className="mt-2 text-sm text-white/70">{selected.description}</p>
              <div className="mt-4 flex gap-3">
                <button onClick={() => setToast(`✅ Candidature envoyée pour « ${selected.title} »`)} className="flex-1 rounded-2xl bg-white px-4 py-2 text-sm font-semibold text-[#0a0f1c]">Postuler</button>
                <button onClick={() => setSelected(null)} className="flex-1 rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-sm">Fermer</button>
              </div>
            </motion.div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} className="fixed bottom-6 left-1/2 -translate-x-1/2 rounded-2xl border border-white/10 bg-white/10 px-4 py-2 text-sm backdrop-blur" onAnimationComplete={() => setTimeout(() => setToast(null), 2000)}>
            {toast}
          </motion.div>
        )}
      </AnimatePresence>
